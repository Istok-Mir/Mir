...
class DottedDict:
    ...
    @classmethod
    def from_base_and_override(cls, base: DottedDict, override: dict | None) -> DottedDict:
    ...
    def walk(self, dotted_key: str) -> Generator[Any, None, None]:
        current: Any = self._d
        keys = dotted_key.split('.')
        for key in keys:
            if isinstance(current, dict):
    ...
    def copy(self, dotted_key: str | None = None) -> Any:
        """
        ...
        :returns:   A copy of the value stored at the path, or None if it doesn't exist.
                    ...
                    entire dictionary.
        """
        return deepcopy(self.get(dotted_key))
